...
import survey from "./data/survey.example.json";
import {
...
import { isSupabaseConfigured, submitSurveyResponse } from "./lib/supabaseClient";
...
export default function App() {
  ...
  const visiblePages = useMemo(() => getVisiblePages(survey, answers), [answers]);
  const currentPage = visiblePages[Math.min(currentPageIndex, visiblePages.length - 1)];
  ...
  async function submit() {
    ...
    try {
      ...
      setSubmission({
        status: "submitted",
        message: result.mode === "supabase" ? "提交成功" : "已保存在本地演示数据中"
      });
    } catch (error) {
  ...
  return (
    <main className="app-shell">
      <section className="survey-header">
        ...
        <div className="storage-pill">{isSupabaseConfigured ? "Supabase" : "Local"}</div>
      </section>

      <section className="progress-block" aria-label="填写进度">
        <div className="progress-meta">
          <span>
            第 {currentPageIndex + 1} / {visiblePages.length} 页
          </span>
          <span>{progress}%</span>
        </div>
      ...
      <form className="survey-form" onSubmit={(event) => event.preventDefault()}>
        ...
        <div className="actions">
          <button type="button" className="secondary-button" onClick={goBack} disabled={currentPageIndex === 0}>
            <ArrowLeft size={18} />
            上一页
          </button>
          {isLastPage ? (
